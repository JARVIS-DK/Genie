...
func GoogleAudioGeneration(data GoogleAudioGenerationRequest, db shared.MongoRepositoryFunctions, metaData shared.ApiMetaData) (shared.ResponseStruct, error) {
	...
	location := "global"
	gcsURI := fmt.Sprintf("gs://opsMatrix-blob-storage/generated_audios/%s.wav", randomString)
	...
	publicURL := fmt.Sprintf("https://storage.googleapis.com/opsMatrix-blob-storage/generated_audios/%s.wav", randomString)
