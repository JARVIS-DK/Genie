...
interface UsePresentationGenerationProps {
  navigate: (path: string) => void; // ✅ Replace Next.js router
}
...
export const usePresentationGeneration = (
  ...
  config?: UsePresentationGenerationProps
) => {
  const dispatch = useDispatch();
  const [loadingState, setLoadingState] = useState<LoadingState>(DEFAULT_LOADING_STATE);

  // Fallback navigate if not provided
  const navigate = config?.navigate || ((path: string) => window.location.assign(path));
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (response) {
        ...
        navigate(`/presentation?id=${presentationId}&stream=true`);
      }
    } catch (error: any) {
  ...
  }, [validateInputs, prepareLayoutData, presentationId, outlines, dispatch, selectedTemplate, navigate, setActiveTab]);
