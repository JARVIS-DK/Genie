...
const TemplateLayouts: React.FC<TemplateLayoutsProps> = ({
  ...
  const layouts = getFullDataByTemplateID(template.id);

  const fonts =
    getCustomTemplateFonts(template.id.replace("custom-", "")) || [];

  // Load custom template fonts
  useFontLoader(fonts);

  return (
    <div
      onClick={() => {
        trackEvent(MixpanelEvent.Group_Layout_Selected_Clicked, {
    ...
    >
        ...
        {layouts?.slice(0, 4).map((layout, index) => (
          ...
          >
            ...
            <div className="transform scale-[0.2] flex justify-center items-center origin-top-left w-[500%] h-[500%]">
              <layout.component data={layout.sampleData} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between text-sm text-gray-500">
        <span>{layouts?.length || 0} layouts</span>
