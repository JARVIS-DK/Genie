...
const SidePanel: React.FC<SidePanelProps> = ({
  ...
  // Template layout preview helper
  const { renderSlideContent } = useTemplateLayouts();

  // Auto-open for desktop, controlled on mobile
  useEffect(() => {
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      ...
      const reordered = arrayMove(presentationData.slides, oldIndex, newIndex);

      const updatedSlides = reordered.map((slide: any, index: number) => ({
        ...slide,
        index,
      }));
  ...
  return (
    <>
      ...
      <div
        className={`
          fixed xl:relative h-full z-50 xl:z-auto transition-all duration-300
          ${isOpen ? "ml-0" : "-ml-[300px]"}
          ${isMobilePanelOpen ? "translate-x-0" : "-translate-x-full xl:translate-x-0"}
        `}
      >
        <div className="min-w-[300px] bg-white max-w-[300px] h-[calc(100vh-120px)] rounded-[20px] hide-scrollbar overflow-hidden shadow-xl">
          {/* Top Panel */}
              ...
              <div className="flex items-center gap-4">
                ...
                <ToolTip content="Image Preview">
                  <Button
                    ...
                    onClick={() => !isStreaming && setActive("grid")}
                  >
                    <LayoutList
                      size={20}
                ...
                <ToolTip content="List Preview">
                  <Button
                    ...
                    onClick={() => !isStreaming && setActive("list")}
                  >
                    <ListTree
                      size={20}
                ...
                {isStreaming ? (
                  <>
                    {presentationData.slides.map((slide: any, index: number) => (
                      <div
                        key={slide.id}
                        className="p-3 rounded-lg slide-box"
                      >
                        <span className="font-medium">Slide {index + 1}</span>
                        <p className="text-sm">{slide.content?.title || ""}</p>
                      </div>
                    ))}
                  </>
                ) : (
                  <SortableContext
                    items={presentationData.slides.map((s: any) => s.id)}
                ...
                {isStreaming ? (
                  presentationData.slides.map((slide: any, index: number) => (
                    <div
                      ...
                      className={`cursor-pointer ring-2 p-1 rounded-md transition-all ${
                        selectedSlide === index ? "ring-[#5141e5]" : "ring-gray-200"
                      }`}
                    >
                      <div className="bg-white relative overflow-hidden aspect-video">
                        <div className="absolute bg-gray-100/5 top-0 left-0 w-full h-full" />
                  ...
                  >
                    {presentationData.slides.map((slide: any, index: number) => (
                      <SortableSlide
                        ...
                        selectedSlide={selectedSlide}
                        onSlideClick={onSlideClick}
                        renderSlideContent={(s) => renderSlideContent(s, false)}
                      />
                    ))}
                  </SortableContext>
                )}
