...
const PresentationPage: React.FC<PresentationPageProps> = ({ presentation_id }) => {
  ...
  useEffect(() => {
    if (!loading && !isStreaming && presentationData?.slides?.length) {
      const firstSlide = presentationData.slides[0];
      const layoutId = firstSlide.layout.split(":")[0].split("custom-")[1];
      const fonts = getCustomTemplateFonts(layoutId);
      useFontLoader(fonts || []);
    }
  }, [presentationData, loading, isStreaming]);

  // Presentation mode view
  if (isPresentMode && presentationData?.slides) {
    return (
      <PresentationMode
    ...
    <div className="h-screen flex overflow-hidden flex-col">
      ...
      >
        ...
        <div className="flex-1 h-[calc(100vh-100px)] overflow-y-auto">
          ...
          >
            {!presentationData || loading || !presentationData?.slides?.length ? (
