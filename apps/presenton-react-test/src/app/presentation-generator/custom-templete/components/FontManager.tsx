...
const FontManager: React.FC<FontManagerProps> = ({
  ...
  const fontsNeedingUpload = allUnsupportedFonts.filter(
    (fontName) => !UploadedFonts.some((uploaded) => uploaded.fontName === fontName)
  );
  ...
  const handleFontUpload = async (fontName: string, file: File) => {
    ...
    try {
      const url = await uploadFont(fontName, file);

      if (url && fileInputRefs.current[fontName]) {
        fileInputRefs.current[fontName]!.value = "";
      }
    } finally {
      setUploadingFonts((prev) => {
        const updated = new Set(prev);
        updated.delete(fontName);
        return updated;
      });
    }
  };

  const handleFileInputChange = (fontName: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    <Card className="my-6">
      <CardHeader>
        ...
        <p className="text-sm text-gray-600">
          We couldn't load these fonts automatically. Please upload them manually. Make sure the font name matches exactly.
        </p>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        {fontsData.internally_supported_fonts.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {fontsData.internally_supported_fonts.map((font, idx) => (
                <div
                  key={idx}
        ...
        {fontsNeedingUpload.length > 0 && (
          <div>
            ...
            <div className="space-y-3">
              {fontsNeedingUpload.map((fontName, idx) => (
                <div key={idx} className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-sm font-medium text-orange-800">{fontName}</span>
                      <p className="text-xs text-orange-600 mt-1">Required for presentation</p>
                    </div>

                    <div className="flex items-center gap-2">
                      <input
                        ref={(el) => (fileInputRefs.current[fontName] = el)}
                        type="file"
                        id={`global-font-upload-${idx}`}
          ...
          <div>
            ...
            <div className="space-y-2">
              {UploadedFonts.map((font, idx) => (
                ...
                >
                  <div>
                    <span className="text-sm font-medium text-green-800">{font.fontName}</span>
                    <p className="text-xs text-green-600 mt-1">Available for all slides</p>
                  </div>
        ...
        <div className="flex justify-center mt-4">
          <Button
            ...
            className="text-xs px-8 py-2 font-semibold bg-blue-600 text-white hover:bg-blue-700 border-blue-600"
          >
            Extract Template
          </Button>
        </div>
