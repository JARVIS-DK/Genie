...
export const SlideContentDisplay: React.FC<SlideContentDisplayProps> = ({
  ...
  // Hide the entire content while editing HTML
  if (isHtmlEditMode) {
    return null;
  }

  // ✅ Processing Screen (Skeleton + Timer)
  if (slide.processing) {
    return (
  ...
  // ✅ Successfully Processed Slide with HTML
  if (slide.processed && slide.html) {
    return (
      <div className="relative">
        {/* Converting HTML → React preview status */}
        ...
        <div ref={slideDisplayRef} className="relative mx-auto w-full">
          ...
          {/* Drawing Canvas (only in edit mode) */}
          {isEditMode && (
            <canvas
              ref={canvasRef}
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="text-sm text-gray-700 p-4 bg-red-50 rounded border border-red-200">
          {slide.error.includes("image exceeds 5 MB maximum") ? (
            <>
              ...
              <p>This slide's image exceeds the 5MB limit. Try using a smaller resolution.</p>
            </>
          ) : (
            slide.error
          )}
        ...
        <div className="flex justify-center">
          <button
            className="bg-red-50 flex gap-2 items-center rounded border border-red-200 px-4 py-2"
