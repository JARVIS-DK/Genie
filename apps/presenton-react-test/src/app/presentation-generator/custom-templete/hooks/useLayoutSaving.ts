...
import { ApiResponseHandler } from "../../services/api/api-error-handler"; // ✅ changed local import
...
export const useLayoutSaving = (
  ...
  const convertSlideToReact = async (
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        retryCount++;
        console.error(
          `Error converting slide ${slide.slide_number} (attempt ${retryCount}):`,
          error
        );

        if (retryCount < maxRetries) {
          toast.error(
            `Failed to convert slide ${slide.slide_number}. Retrying in 2 minutes...`,
            {
              description: `Attempt ${retryCount}/${maxRetries}. Error: ${
          ...
          throw new Error(
            `Failed to convert slide ${slide.slide_number} after ${maxRetries} attempts: ${
              error instanceof Error
                ? error.message
                : "An unexpected error occurred"
            }`
  ...
  const saveLayout = useCallback(
    async (layoutName: string, description: string): Promise<string | null> => {
      if (!slides.length) {
      ...
      try {
        ...
        const googleFontCssUrls =
          fontsData?.internally_supported_fonts
            ?.map((f) => f.google_fonts_url)
            .filter(Boolean) || [];

        const FontUrls = Array.from(
          new Set([...(uploadedFontUrls || []), ...googleFontCssUrls])
        );
        ...
        for (let i = 0; i < slides.length; i++) {
          ...
          setSlides((prev) =>
            prev.map((s, idx) =>
              idx === i ? { ...s, convertingToReact: true } : s
            )
          );

          try {
            const reactComponent = await convertSlideToReact(
              slide,
              presentationId,
              FontUrls
            );
            ...
            console.error(
              `Error converting slide ${slide.slide_number}:`,
              error
            );
            toast.error(
              `Failed to convert slide ${slide.slide_number} after all retries`,
              {
                description:
                  error instanceof Error
                    ? error.message
            ...
            setSlides((prev) =>
              prev.map((s, idx) =>
                idx === i ? { ...s, convertingToReact: false } : s
              )
            );
          }
        }
