...
const CustomTemplatePage: React.FC = () => {
  ...
  const handleSaveTemplate = async (layoutName: string, description: string): Promise<string | null> => {
    ...
    if (id) {
      navigate(`/template-preview/custom-${id}`);
    }
  ...
  // Load tailwind script if not present
  useEffect(() => {
    const existing = document.querySelector('script[src*="tailwindcss.com"]');
    if (!existing) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="max-w-[1440px] aspect-video mx-auto px-6">
        ...
        <div className="text-center space-y-2 my-6">
          <h1 className="text-4xl font-bold text-gray-900">Custom Template Processor</h1>

          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Upload your PDF or PPTX file to extract slides and convert them to a reusable AI template.
          </p>

          <div className="inline-block rounded border border-orange-200 bg-orange-50 px-3 py-2 text-sm text-orange-700 mt-2">
          ...
          <div className="space-y-6 mt-10">
            {slides.map((slide, index) => (
              <EachSlide
                ...
                setSlides={setSlides}
                onSlideUpdate={(updated) => handleSlideUpdate(index, updated)}
              />
            ))}
          </div>
        )}
