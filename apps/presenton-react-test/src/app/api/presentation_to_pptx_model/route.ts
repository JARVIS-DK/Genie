...
router.get("/", async (req: Request, res: Response) => {
  ...
  try {
    const id = getPresentationId(req);
    [browser, page] = await getBrowserAndPage(id);
    const screenshotsDir = getScreenshotsDir();

    const { slides, speakerNotes } = await getSlidesAndSpeakerNotes(page);
    const slidesAttributes = await getSlidesAttributes(slides, screenshotsDir);

    await postProcessSlidesAttributes(slidesAttributes, screenshotsDir, speakerNotes);

    const pptSlides = convertElementAttributesToPptxSlides(slidesAttributes);

    const response: PptxPresentationModel = {
      slides: pptSlides,
    };
...
function getPresentationId(req: Request) {
...
function getScreenshotsDir() {
  ...
  if (!tempDir) throw new ApiError("TEMP_DIRECTORY environment variable not set");

  const folder = path.join(tempDir, "screenshots");
...
async function postProcessSlidesAttributes(
  ...
  for (const [index, slide] of slidesAttributes.entries()) {
    for (const element of slide.elements) {
      if (element.should_screenshot) {
        const file = await screenshotElement(element, screenshotsDir);
        element.imageSrc = file;
        element.should_screenshot = false;
        element.objectFit = "cover";
        element.element = undefined;
      }
    }
    slide.speakerNote = speakerNotes[index];
  }
}
...
async function screenshotElement(
  ...
  const screenshotPath = path.join(screenshotsDir, `${uuidv4()}.png`);

  if (element.tagName === "svg") {
    const png = await convertSvgToPng(element);
  ...
  await element.element?.evaluate((el) => {
    ...
    const hideAll = (target: Element) => {
      const all = document.querySelectorAll("*");
      all.forEach((elem) => {
        const style = window.getComputedStyle(elem);
        originalOpacities.set(elem, style.opacity);

        if (target === elem || target.contains(elem) || elem.contains(target)) {
          (elem as HTMLElement).style.opacity = style.opacity || "1";
        } else {
  ...
  await element.element?.screenshot({ path: screenshotPath });

  await element.element?.evaluate((el) => {
    if ((el as any).__restore) (el as any).__restore();
  });

  return screenshotPath;
}

async function convertSvgToPng(element: ElementAttributes) {
  const svgHtml =
    (await element.element?.evaluate((el) => el.outerHTML)) || "<svg></svg>";

  const buf = await sharp(Buffer.from(svgHtml))
    .resize(
...
async function getSlidesAndSpeakerNotes(page: Page) {
  const wrapper = await page.$("#presentation-slides-wrapper");
  if (!wrapper) throw new ApiError("Presentation slides not found");

  const speakerNotes = await wrapper.evaluate((el) =>
    Array.from(el.querySelectorAll("[data-speaker-note]")).map(
      (node) => node.getAttribute("data-speaker-note") || ""
    )
  );

  const slides = await wrapper.$$(":scope > div > div");
