...
function transformSvg(svgText: string, options: RemoteSvgOptions): string {
  try {
    ...
    rects.forEach(r => {
      ...
      const isExact = wAttr === "100%" && hAttr === "100%" && (xAttr === "0" || xAttr === "0%") && (yAttr === "0" || yAttr === "0%");
      const approx = vbW > 0 && vbH > 0 &&
...
function makeCacheKey(url: string, options: RemoteSvgOptions) {
  return `${url}|sc=${options.strokeColor || ""}|fc=${options.fillColor || ""}|cls=${options.className || ""}`;
}
  ...
  React.useEffect(() => {
    ...
    async function fetchSvg() {
      ...
      const fallback = () => {
        ...
        return `<svg${cls} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke="${stroke}" fill="${fill}" stroke-width="2"><circle cx="12" cy="12" r="10" fill="currentColor" opacity="0.12"></circle></svg>`;
      };
      ...
      try {
        ...
        if (!ct.includes("svg")) throw new Error(`Invalid content: ${ct}`);

        const text = await res.text();
        if (cancelled) return;

        const transformed = transformSvg(text, options);
        cacheSet(key, transformed);
